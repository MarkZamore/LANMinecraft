...
/// <summary>
/// End-to-end sync against the real release the built-in pack is fetched from.
...
/// compare the synced tree against (defaults to the folder under Documents named
/// after the built-in pack); when the path does not exist the comparison is skipped.
/// </summary>
    ...
    public async Task FullInstall_TreeMatchesRepository_IncrementalRepairsSingleFile()
    {
        ...
        var repo = Environment.GetEnvironmentVariable("PACKSYNC_E2E_REPO")
            ?? Path.Combine(
                ...
                "Documents", PortablePackSyncService.DefaultPackRelativePath);
